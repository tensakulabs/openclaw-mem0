/**
 * mem0ai OSS module — vendored and patched
 *
 * Source: mem0ai@2.2.2 (dist/oss/index.mjs)
 * Patches applied:
 *   1. Provider imports converted to lazy dynamic imports (no crash on missing SDKs)
 *   2. OpenAIEmbedder: passes baseURL to OpenAI client (fixes OpenRouter/DashScope)
 *   3. ConfigManager: includes baseURL in embedder config merge
 *   4. QdrantClient: disables version compatibility warning
 *
 * Only install the provider SDKs you actually use.
 * Minimum for self-hosted: openai, @qdrant/js-client-rest, sqlite3
 */
...
let OpenAI; try { OpenAI = (await import("openai")).default; } catch {}
var OpenAIEmbedder = class {
  constructor(config) {
    const openaiOpts = { apiKey: config.apiKey }; if (config.baseURL) openaiOpts.baseURL = config.baseURL; this.openai = new OpenAI(openaiOpts);
...
let Ollama; try { ({ Ollama } = await import("ollama")); } catch {}
...
let OpenAI2; try { OpenAI2 = (await import("openai")).default; } catch {}
var OpenAILLM = class {
  constructor(config) {
...
let OpenAI3; try { OpenAI3 = (await import("openai")).default; } catch {}
...
let Anthropic; try { Anthropic = (await import("@anthropic-ai/sdk")).default; } catch {}
...
let Groq; try { ({ Groq } = await import("groq-sdk")); } catch {}
var GroqLLM = class {
  constructor(config) {
...
let Mistral; try { ({ Mistral } = await import("@mistralai/mistralai")); } catch {}
...
let sqlite3; try { sqlite3 = (await import("sqlite3")).default; } catch {}
...
let QdrantClient; try { ({ QdrantClient } = await import("@qdrant/js-client-rest")); } catch {}
...
var Qdrant = class {
  constructor(config) {
    if (config.client) {
      ...
      params.checkCompatibility = false;
...
let Cloudflare; try { Cloudflare = (await import("cloudflare")).default; } catch {}
...
let createClient; try { ({ createClient } = await import("redis")); } catch {}
var DEFAULT_FIELDS = [
...
let Ollama2; try { Ollama2 = (await import("ollama")).Ollama; } catch {}
var OllamaLLM = class {
  constructor(config) {
    // Using this variable to avoid calling the Ollama server multiple times
    this.initialized = false;
...
let createClient2; try { createClient2 = (await import("@supabase/supabase-js")).createClient; } catch {}
...
let sqlite32; try { sqlite32 = (await import("sqlite3")).default; } catch {}
...
let createClient3; try { createClient3 = (await import("@supabase/supabase-js")).createClient; } catch {}
...
let GoogleGenAI; try { ({ GoogleGenAI } = await import("@google/genai")); } catch {}
...
let GoogleGenAI2; try { GoogleGenAI2 = (await import("@google/genai")).GoogleGenAI; } catch {}
var GoogleLLM = class {
  constructor(config) {
...
let AzureOpenAI; try { ({ AzureOpenAI } = await import("openai")); } catch {}
...
let AzureOpenAI2; try { AzureOpenAI2 = (await import("openai")).AzureOpenAI; } catch {}
...
let AIMessage, HumanMessage, SystemMessage;
try {
  ({ AIMessage, HumanMessage, SystemMessage } = await import("@langchain/core/messages"));
} catch {}
...
let Document; try { ({ Document } = await import("@langchain/core/documents")); } catch {}
...
let SearchClient, SearchIndexClient, AzureKeyCredential;
try {
  ({ SearchClient, SearchIndexClient, AzureKeyCredential } = await import("@azure/search-documents"));
} catch {}
let DefaultAzureCredential; try { ({ DefaultAzureCredential } = await import("@azure/identity")); } catch {}
...
let neo4j; try { neo4j = (await import("neo4j-driver")).default; } catch {}
